...
double alpha_beta_tp(double alpha, double beta, int depth, bool is_white) {
	...
	if (depth == MAX_DEPTH || chk_mate(is_white) || chk_mate(!is_white)) {
		...
		if (!(transpositions[index].valid)) {
			...
			transpositions[index].depth = depth;
		...
		for (int j = 0; j < 64; j++) {
			...
			if (move) {
				...
				if (transpositions[index].valid && (transpositions[index].key == zobrist_hash(is_white)) &&  (depth == transpositions[index].depth)) {
					...
					score = -alpha_beta_tp(-beta, -alpha, depth + 1, !is_white);
				}
				wP = wP0;
				wKn = wKn0;
				wB = wB0;
	...
	if (vk_castle->elements[0] && vk_castle->elements[1]) {
		if ((is_white && wk_castle) || (!is_white && bk_castle)) {
			...
			if (transpositions[index].valid && (transpositions[index].key == zobrist_hash(is_white)) && (transpositions[index].depth == depth)) {
				...
				k_score = -alpha_beta_tp(-beta, -alpha, depth + 1, !is_white);
			}
			wP = wP0;
			wKn = wKn0;
			wB = wB0;
	...
	if (vq_castle->elements[0] && vq_castle->elements[1]) {
		if ((is_white && wq_castle) || (!is_white && bq_castle)) {
			...
			if (transpositions[index].valid && (transpositions[index].key == zobrist_hash(is_white)) && (transpositions[index].depth == depth)) {
				...
				q_score = -alpha_beta_tp(-beta, -alpha, depth + 1, !is_white);
			}
			wP = wP0;
			wKn = wKn0;
			wB = wB0;
	...
	for (int i = 0; i < (v->size); i++) {
		...
		for (int j = 0; j < 64; j++) {
			...
			if (move) {
				...
				mk_move(move, v->origin[i], is_white, v->piece[i]);
				if (use_transpose) {
					alpha_root = -alpha_beta_tp(-INFINITY, INFINITY, 0, !is_white);
				} else {
